...
{
    public class ProfileCommentUpvotedUserRepository : Repository<ProfileCommentUpvotedUser>, IProfileCommentUpvotedUserRepository
    {
        public ProfileCommentUpvotedUserRepository(IYamaancoDbContext context)
            : base(context)
        {
        }
        ...
        public async Task<CommentUpvotedDto> UpdateCommentUpvoteCommand(string commentId, string upvotedUserId)
        {
            ...
            try
            {
                ...
                commentUpvoted = await db.QueryFirstAsync<CommentUpvotedDto>(@" [spUpdateProfileCommentUpvoteCommand] @CommentId,@UpvotedUserId", @params);
            }
            catch (Exception ex)
            {
                throw ex;
