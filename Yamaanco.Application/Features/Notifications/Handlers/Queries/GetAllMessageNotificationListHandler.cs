...
{
    public class GetAllMessageNotificationListHandler : IRequestHandler<GetAllMessageNotificationListQuery, PagedResponse<IEnumerable<AllNotificationsListView>>>
    {
        private readonly INotificationsRepository _saredNotificationsCollection;
        private IElapsedTime _elapsedTime;
        private readonly IAccountService _accountService;

        public GetAllMessageNotificationListHandler(IElapsedTime elapsedTime,
 INotificationsRepository saredNotificationsCollection, IAccountService accountService)
        ...
        public async Task<PagedResponse<IEnumerable<AllNotificationsListView>>> Handle(GetAllMessageNotificationListQuery request, CancellationToken cancellationToken)
        {
            var currentUser = _accountService.GetCurrentUser();
            var notificationList = new List<AllNotificationsListView>();
            var response = await _saredNotificationsCollection
                .GetAllMessageNotifications(currentUser.Id,
