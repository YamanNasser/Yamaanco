...
{
    public class GetUnSeenGeneralNotificationListHandler : IRequestHandler<GetUnSeenGeneralNotificationListQuery, PagedResponse<IEnumerable<UnSeenNotificationsListView>>>
    {
        private readonly INotificationsRepository _saredNotificationsCollection;
        private IElapsedTime _elapsedTime;
        private readonly IAccountService _accountService;

        public GetUnSeenGeneralNotificationListHandler(IElapsedTime elapsedTime,
 INotificationsRepository saredNotificationsCollection, IAccountService accountService)
        ...
        public async Task<PagedResponse<IEnumerable<UnSeenNotificationsListView>>> Handle(GetUnSeenGeneralNotificationListQuery request, CancellationToken cancellationToken)
        {
            var currentUser = _accountService.GetCurrentUser();
            var notificationList = new List<UnSeenNotificationsListView>();

            var response = await _saredNotificationsCollection
                .GetUnSeeGeneralNotifications(currentUser.Id,
