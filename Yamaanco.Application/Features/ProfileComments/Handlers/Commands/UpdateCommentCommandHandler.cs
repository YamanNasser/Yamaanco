...
namespace Yamaanco.Application.Features.ProfileComments.Handlers.Commands
{
    public class UpdateCommentCommandHandler : IRequestHandler<UpdateCommentCommand, Response<string>>
    {
        ...
        public UpdateCommentCommandHandler(IUnitOfWork unitOfWork, IFileService fileService, IOptions<AppOptions> appSettings, IMediator mediator, IAccountService accountService)
        {
        ...
        public async Task<Response<string>> Handle(UpdateCommentCommand request, CancellationToken cancellationToken)
        {
            ...
            var comment = await _unitOfWork.ProfileCommentRepository.GetProfileCommentIncludePhotoResources(request.CommentId);
        ...
        private async Task SendNotification(ProfileComment comment, CancellationToken cancellationToken)
        {
        ...
        private static List<CommentResourcesDto> MapResourcesAsCommentResourcesDto(IEnumerable<ProfileCommentResources> resources)
        {
        ...
        private async Task<List<ProfileCommentResources>> GetNewResources(UpdateCommentCommand request, ProfileComment comment)
        {
